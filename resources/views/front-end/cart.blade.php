...
<div class="row">
  @if (Cart::getTotalQuantity()>0)
    <div class="col l9"><h5>Giỏ hàng ({{Cart::getTotalQuantity()}} sản phẩm)</h5>
            ...
            <tbody>
                @foreach (Cart::getContent() as $row)
                  <tr>
                      ...
                      <td>
                          ...
                          @endif
                              <p class="" style="font-size: 13px">
                                <a href="javascript:void(0);" class="btn-item-delete" data-product-id="{{$row->id}}">
                                    Xóa
                                </a>
                                @if (!Auth::guest())
                                 &nbsp;| &nbsp;
                      ...
                      <td><input type="number" name="rowIds[{{$row->id}}]" value="{{$row->quantity}}" class="number-format"></td>
                      <td><span class="number-format">{{$row->price}}</span> <span> &nbsp;₫</span></td>
                      <td><span class="number-format">{{$row->price*$row->quantity}}</span> <span> &nbsp;₫</span></td>
                  </tr>
                  {{-- expr --}}
                @endforeach
            </tbody>
    ...
    <div class="col l3">
      <table style="border:1px solid #f2f2f2;border-radius: 4px;">
            <tbody>
              <tr>
                <td>Tổng tiền</td>
                <td><span class="number-format">{{Cart::getSubTotal()}}</span> <span> &nbsp;₫</span></td>
              </tr>
              {{-- <tr>
                <td>Thuế:</td>
                <td><span class="number-format">{{Cart::getSubTotalWithoutConditions()}}</span> <span> &nbsp;₫</span></td>
              </tr> --}}
              <tfoot style="border-top: 2px solid red">
                <td>Thành tiền </td>
                <td>
                  <h5 class="red-text " style="font-size:16px;">
                    <span class="number-format">{{Cart::getTotal()}}</span><span> &nbsp;₫</span>
                  </h5> 
                </td>
              </tfoot>
            </tbody>
...
@foreach (Cart::getContent() as $row)
  ...
  <form action="{{ route('cart.destroy',$row->id) }}" data-form-id='{{$row->id}}' method="POST" style="display: none;">
  @method('DELETE')
  @csrf
  </form>
@endforeach
